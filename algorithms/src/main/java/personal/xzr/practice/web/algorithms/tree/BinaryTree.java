...
public class BinaryTree {
	...
	/**
	 * 用于检测二叉树广义表表达式合法性的编译了的正则表达式: 合法的广义表表达式可以是： [1] 基本表达式： A，A(,), A(B,),
	 * A(,B), A(B,C) [2] 上述的 A，B，C 可以是 [1] 中的基本表达式 [3] A,B,C
	 * 可允许的取值范围由应用决定，这里仅允许是 大小写字母，数字，+-/*% [4]
	 * 表达式中不含任何空格符，因此，在检查表达式之前，必须确保表达式中不含空格符
	 * 
	 */
	private static String permittedChars = "[a-zA-Z0-9//+//-//*/////%]";
	private static String basicUnit = "[a-zA-Z0-9//+//-//*/////%//(//),]";
	private static Pattern basicPattern = Pattern.compile("" + "|" + permittedChars + "|" + permittedChars + "//("
			+ permittedChars + "?," + permittedChars + "?//)?");
	private static Pattern extendPattern = Pattern
			.compile(permittedChars + "//(" + basicUnit + "*," + basicUnit + "*//)");
	...
	/**
	 * equalsTo: 比较该二叉树与给定二叉树 another 是否全等； 若全等则返回 true, 否则返回 false.
	 */
	...
	/**
	 * equalsTo : 比较给定的两个二叉树是否全等 两个二叉树全等当且仅当 A. 两个二叉树均为空; 或者 B.
	 * 两个二叉树均非空，且所有对应位置的结点都相同，对应结点之间的关联也完全相同.
	 */
	private boolean compareEqual(TreeNode root, TreeNode anotherRoot) {
		return (root == null && anotherRoot == null) || ((root != null && anotherRoot != null)
				&& (root.getCh() == anotherRoot.getCh()) && (compareEqual(root.getLchild(), anotherRoot.getLchild()))
	...
	private static String getGeneralList(String inOrderList, String postOrderList, boolean flag) {
		...
		if (inOrderList.length() > 0 || postOrderList.length() > 0) {
			...
			s += ch;
			s += "(" + getGeneralList(inOrderLeft, postOrderLeft, true) + ","
					+ getGeneralList(inOrderRight, postOrderRight, true) + ")";
		}
		return s;
	}
	...
	/**
	 * separatorIndex : 求解广义表表达式中分割左右子树的分隔符的位置 由于这里使用逗号分割左右子树，则当且仅当其位置应当满足：
	...
	private static int separatorIndex(String expression) {
		...
		for (; index < expression.length(); index++) {
			...
			if (ch == '(') {
				...
				if (leftBracketCounter == rightBacketCounter + 1)
					break;
			}
		}
